...
// ISO stuff follows
    ...
    */     
    ...
    {
        def otherDate = new Date();
        say "... DateSupport.getIsoDate() today's Date is=|${otherDate.toString()}|"
        ...
        try
        {
            ...
            say "... DateSupport tokens= tokens[2]=|${tokens[2]}| tokens[1]=|${tokens[1]}| tokens[0]=|${tokens[0]}| "
            say "... DateSupport tokens= intDay=|${intDay}| intMo=|${intMo}| intYr=|${intYr}| "

            // don't let years thru is before 1900 or after 2100
            if (intYr > 0 && intYr < 100) { intYr += 2000; }                 

            say "... otherDate today's date is =|${otherDate.toString()}|"              
            otherDate[YEAR] = intYr;
            otherDate[MONTH] = intMo;
            otherDate[DATE] = intDay;
            say "... otherDate=|${otherDate.toString()}| after applying tokens"
            otherDate[MONTH] = intMo - 1;
            say "... otherDate=|${otherDate.toString()}| after subtracting one month"
...
"""
    ...
    */     
    public static void main(String[] args)
    {
        ...
        String ans = obj.getIsoDate().toString();
		obj.say "... ok=${ok} for 2017-09-21 ans = |${ans}|"
        assert ok == true;
        assert ans.startsWith("Thu Sep 21 ") == true

        obj.say "... to convert dd/mm/yyyy strings into a Date() object"

        obj.say "... today is "+obj.datex;
        if (obj.isDate(obj.datex))
        {
            obj.say "getDate()="+obj.getDate();
        }

        ans = obj.isDate("29/09/2017");
        obj.say "... and answer was "+ans;
        if (ans)
        {
            obj.say "getDate()="+obj.getDate();
        }
        ...
        // ==================================================
        obj.say ""
        obj.say "... try ISO dates -"

        obj.say "... today is "+obj.isodate;
        if (obj.isIsoDate(obj.isodate))
        {
            obj.say "getIsoDate()="+obj.getIsoDate();
        }
        ...
        ["1921-10-31","2017/12/31","25.12.2018","2017.12.25", "2 17-4-4", ",", "//","1/1/1", " "].each{e->
            obj.say "... is [${e}] valid ? "+obj.hasValidSeparator(e)+" separatorType=${obj.separatorType} how many tokens ? ${obj.tokens.size()}"
        } // end of each

        ["Hi kids?=","23","0123456789","1 2345","678\n89"].each{v-> 
            obj.say ' '+obj.validate(v);
        } // end of each
